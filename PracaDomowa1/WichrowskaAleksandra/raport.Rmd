...
```{r preprocessing, include = TRUE, echo=TRUE}
...
```{r onehot, include=TRUE, echo=TRUE}
...
Next look into correlation matrix. 

```{r correlation}
correlation_matrix <- cor(sapply(dataset, as.numeric, MARGIN=2), use = "pairwise.complete.obs")
```

```{r corrplot, include=TRUE, echo=TRUE}
corrplot(correlation_matrix)
```

We can find two correlated variables: FTI_measured and T4U_measured.
Correlation:
``` {r delete_correlated, include=TRUE, echo=TRUE}
...
```{r split}
split_index <- read.csv2('indeksy_treningowe.txt')$x
...
r = resample(learner_rpart_tuned, task_rpart, cv5, list(mlr::auc, auprc))
```
